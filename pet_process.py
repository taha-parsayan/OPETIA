...
def btn_process_command():
    if var_file_address_input.get() == "Not defined...":
        ...
        for vol_name in pet_vols:
            ...
            myfunctions.FSL_Brain_Extraction(input_file, output_file, var_BET_threshold.get(), var_BET_function.get())
        print('Done')
...
frame4.place(x=5, y=560, width=490, height=80)
...
label5 = tk.Label(frame3, text = "Brain extracted T1 in native space (structural_brain.nii.gz):").place(x=5, y=5)
...
label7 = tk.Label(frame3, text = "Standard template (MNI152_T1_2mm_brain.nii.gz):").place(x=5, y=115)
...
label8 = tk.Label(frame3, text = "PET to T1 registration model (DOF):").place(x=5, y=170)
...
label9 = tk.Label(frame3, text = "PET to T1 Cost function:").place(x=5, y=200)
...
label10 = tk.Label(frame3, text = "PET to T1 interpolation method:").place(x=5, y=230)
